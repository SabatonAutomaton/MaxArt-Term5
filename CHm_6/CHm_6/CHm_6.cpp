...
int* result = nullptr;
int* fict = nullptr;
int* kray_func = nullptr;
int* kray_type = nullptr;
...
double u_betta(double x, double y, int choise)
{
	switch (choise)
	{
	case 1:
...
static void read(int choise)
{
	...
	kray_type = new int[n_x * n_y] {0};
	kray_func = new int[n_x * n_y] {0};
		...
		for (int i = x_a; i <= x_b; i++)
		{
			for (int j = y_a; j <= y_b; j++)
			{
				...
				else if (type == 3)
				{
					...
					if (kray_type[k] != 1 && kray_type[k] != 3)
					{
						kray_type[k] = 2;
						kray_func[k] = number_func;
					}
...
void build_matrix()
{
	...
	for (int i = 0; i < n_x; i++)
	{
		...
		{
			A[2][i] = 1;
			vect[i] = func_kray(mesh_x[i], mesh_y[0], kray_func[i]);
		}
		else if (kray_type[i] == 2)
		{
			double h = mesh_y[1] - mesh_y[0];
			A[2][i] = lambda / h;
			A[4][i] = -lambda / h;
			vect[i] = theta(mesh_x[i], mesh_y[0], kray_func[i]);
		}
		else if (kray_type[i] == 3)
		{
			double h = mesh_y[1] - mesh_y[0];
			A[2][i] = lambda / h + betta;
			A[4][i] = -lambda / h;
			vect[i] = u_betta(mesh_x[i], mesh_y[0], kray_func[i]);
		}

		// up kray
		if (kray_type[n_x * n_y - i - 1] == 1)
		{
			A[2][n_x * n_y - i - 1] = 1;
			vect[n_x * n_y - i - 1] = func_kray(mesh_x[n_x - i - 1], mesh_y[n_y - 1], kray_func[n_x * n_y - i - 1]);
		}
		else if (kray_type[n_x * n_y - i - 1] == 2)
		{
			double h = mesh_y[n_y - 1] - mesh_y[n_y - 2];
			A[0][n_x * n_y - i - 1 + ind[0]] = -lambda / h;
			A[2][n_x * n_y - i - 1] = lambda / h;
			vect[n_x * n_y - i - 1] = theta(mesh_x[n_x - i - 1], mesh_y[n_y - 1], kray_func[n_x * n_y - i - 1]);
		}
		else if (kray_type[n_x * n_y - i - 1] == 3)
		{
			double h = mesh_y[n_y - 1] - mesh_y[n_y - 2];
			A[0][n_x * n_y - i - 1 + ind[0]] = -lambda / h;
			A[2][n_x * n_y - i - 1] = lambda / h + betta;
			vect[n_x * n_y - i - 1] = u_betta(mesh_x[n_x - i - 1], mesh_y[n_y - 1], kray_func[n_x * n_y - i - 1]);
		}
	}
	...
	for (int i = 1; i < n_y - 1; i++)
	{
		...
		// left kray
		if (kray_type[n_x * i] == 1)
		{
			A[2][n_x * i] = 1;
			vect[n_x * i] = func_kray(mesh_x[0], mesh_y[i], kray_func[n_x * i]);
		}
		else if (kray_type[n_x * i] == 2)
		{
			double h = mesh_x[1] - mesh_x[0];
			A[2][n_x * i] = lambda / h;
			A[3][n_x * i] = -lambda / h;
			vect[n_x * i] = theta(mesh_x[0], mesh_y[i], kray_func[n_x * i]);
		}
		else if (kray_type[n_x * i] == 3)
		{
			double h = mesh_x[1] - mesh_x[0];
			A[2][n_x * i] = lambda / h + betta;
			A[3][n_x * i] = -lambda / h;
			vect[n_x * i] = u_betta(mesh_x[0], mesh_y[i], kray_func[n_x * i]);
		}

		// right kray
		if (kray_type[n_x * i + n_x - 1] == 1)
		{
			A[2][n_x * i + n_x - 1] = 1;
			vect[n_x * i + n_x - 1] = func_kray(mesh_x[n_x - 1], mesh_y[i], kray_func[n_x * i + n_x - 1]);
		}
		else if (kray_type[n_x * i + n_x - 1] == 2)
		{
			double h = mesh_x[n_x - 1] - mesh_x[n_x - 2];
			A[1][n_x * i + n_x - 1 + ind[1]] = -lambda / h;
			A[2][n_x * i + n_x - 1] = lambda / h;
			vect[n_x * i + n_x - 1] = theta(mesh_x[n_x - 1], mesh_y[i], kray_func[n_x * i + n_x - 1]);
		}
		else if (kray_type[n_x * i + n_x - 1] == 3)
		{
			double h = mesh_x[n_x - 1] - mesh_x[n_x - 2];
			A[1][n_x * i + n_x - 1 + ind[1]] = -lambda / h;
			A[2][n_x * i + n_x - 1] = lambda / h + betta;
			vect[n_x * i + n_x - 1] = u_betta(mesh_x[n_x - 1], mesh_y[i], kray_func[n_x * i + n_x - 1]);
		}
		...
		for (int j = 1; j < n_x - 1; j++)
		{
			...
			if (fict[k] == 2)
			{
				...
				{
					A[2][k + ind[2]] = 1;
					vect[k] = func_kray(mesh_x[j], mesh_y[i], kray_func[k]);
				}
				else if (kray_type[k] == 2)
				{
					//left
					if (fict[k - 1] == 1)
					{
						...
						A[2][k] = lambda / h;
						A[3][k] = -lambda / h;
						vect[k] = theta(mesh_x[j], mesh_y[i], kray_func[k]);
					}
					// right
					else if (fict[k + 1] == 1)
					{
				...
				else if (kray_type[k] == 3)
				{
					//left
					if (fict[k - 1] == 1)
					{
						double h = mesh_x[j + 1] - mesh_x[j];
						A[2][k] = lambda / h + betta;
						A[3][k] = -lambda / h;
						vect[k] = u_betta(mesh_x[j], mesh_y[i], kray_func[k]);
					}
					// right
					else if (fict[k + 1] == 1)
					{
